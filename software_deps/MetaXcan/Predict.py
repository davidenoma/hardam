...
from software_deps.MetaXcan import metax
from software_deps.MetaXcan.metax import Exceptions, Logging, Utilities, PredictionModel
from software_deps.MetaXcan.metax import BGENGenotype, DosageGenotype
from software_deps.MetaXcan.metax.genotype import Genotype
from software_deps.MetaXcan.metax import GWASAndModels, Genomics
from software_deps.MetaXcan.metax.misc import KeyedDataSource
...
def dosage_generator(args, variant_mapping=None, weights=None):
    ...
    if args.text_genotypes:
        ...
        from software_deps.MetaXcan.metax.genotype import CYVCF2Genotype
...
def load_samples(args):
    ...
    if args.text_sample_ids:
        ...
        from software_deps.MetaXcan.metax import BGENGenotype
...
def prepare_prediction(args, extra, samples):
    ...
    if len(args.prediction_output) < 2:
        from software_deps.MetaXcan.metax.predixcan.Utilities import BasicPredictionRepository
        results = BasicPredictionRepository(samples, extra, args.prediction_output[0])
    else:
        if args.prediction_output[1] == "HDF5":
            from software_deps.MetaXcan.metax.predixcan.Utilities import HDF5PredictionRepository
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Predict.py %s: Predict transcriptome from a model.' % (
        metax.__version__))
